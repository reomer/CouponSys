...
public class CustomerFacade  extends ClientsFacade{
	...
	public void purchaseCoupon(Coupon coupon) throws IDDoesntExistException, NotAllowedException {
		...
		if (couponBought == true) {
			throw new NotAllowedException("Sorry, Cannoy buy the same coupon twice.");
		}

		if (this.couponsDAO.getOneCoupon(coupon.getId()).getAmount() == 0) { // Did coupon still in store?
			throw new NotAllowedException("Sorry, this coupon is no longer avaliable.");
		} else if (this.couponsDAO.getOneCoupon(coupon.getId()).getEndDate() // Did coupon expire today?
				.compareTo(java.sql.Date.valueOf(LocalDate.now())) == 0) {
			throw new NotAllowedException("Sorry, cannot purchase. This coupon expiere today.");
