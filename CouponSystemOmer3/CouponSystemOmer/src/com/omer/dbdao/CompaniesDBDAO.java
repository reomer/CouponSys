...
public class CompaniesDBDAO implements CompaniesDAO {
	...
	private static final String UPDATE_QUERY ="UPDATE `coupon_sys`.`COMPANIES` SET `name`=?, `email`=?,`password`=? WHERE `id`=?;\n" + 
			"";
	private static final String DELETE_QUERY ="DELETE FROM `coupon_sys`.`COMPANIES` WHERE `id`='1';\n" + 
			"";
	private static final String GET_ONE_COMPANY_BY_ID = "SELECT * FROM coupon_sys.COMPANIES WHERE (`ID` = ?)";
	...
	private static final String IS_COMPANY_EXIST_QUERY ="SELECT*FROM coupon_sys.COMPANIES WHERE id=?";
